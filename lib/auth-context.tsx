...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    if (userDoc.exists()) {
      ...
      if (userData.familyId) {
        ...
        if (familyDoc.exists()) {
          ...
          // Convert all Firestore timestamps to Date
          const dateFields = ["createdAt", "trialStartedAt", "trialEndsAt", "trialEndedAt", "accountLockedAt", "scheduledDeletionAt"] as const
          for (const field of dateFields) {
            if (familyData[field] && typeof familyData[field] === "object" && "toDate" in (familyData[field] as object)) {
              (familyData as Record<string, unknown>)[field] = (familyData[field] as { toDate: () => Date }).toDate()
            }
          }
  ...
  const signUp = async (data: SignUpFormData) => {
    ...
    // Create family document with trial
